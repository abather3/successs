...
class AnalyticsExportService {
    ...
    static async exportAnalytics(options) {
        ...
        let data;
        if (type === 'hourly') {
            data = await QueueAnalyticsService_1.QueueAnalyticsService.getQueueAnalytics(startDate, endDate);
        }
        else {
            data = await QueueAnalyticsService_1.QueueAnalyticsService.getDailySummaries(startDate, endDate);
        }
        ...
        switch (format) {
            case 'pdf':
                headers = originalHeaders.map(header => this.PDF_HEADER_MAPPINGS[header] || this.HEADER_MAPPINGS[header] || header);
                break;
            case 'sheets':
                headers = originalHeaders.map(header => this.SHEETS_COLUMN_CONFIG[header]?.header || this.HEADER_MAPPINGS[header] || header);
                break;
            default:
     ...
     */
    static formatPDFData(data, headers) {
        const originalHeaders = Object.keys(data[0] || {});
        const rows = data.map(row => originalHeaders.map(header => {
            const value = row[header];
            if (value === null || value === undefined) {
                return '0';
            }
     ...
     */
    static async exportProcessingAnalytics(startDate, endDate, format = 'json') {
        const query = `
      SELECT 
 ...
 */
AnalyticsExportService.HEADER_MAPPINGS = {
    // Base fields
    'date': 'Date',
    'hour': 'Hour',
 ...
 */
AnalyticsExportService.PDF_HEADER_MAPPINGS = {
    'date': 'Date',
    'hour': 'Hr',
 ...
 */
AnalyticsExportService.SHEETS_COLUMN_CONFIG = {
...
//# sourceMappingURL=AnalyticsExportService.js.map