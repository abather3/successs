...
class ExportService {
     ...
     */
    static async exportCustomerToExcel(customerId) {
        try {
     ...
     */
    static async exportCustomerToPDF(customerId) {
        const customer = await customer_1.CustomerService.findById(customerId);
        if (!customer) {
            throw new Error('Customer not found');
        }
     ...
     */
    static async exportCustomerToGoogleSheets(customerId) {
        const customer = await customer_1.CustomerService.findById(customerId);
        if (!customer) {
            throw new Error('Customer not found');
        }
        ...
        const response = await axios_1.default.post(this.GOOGLE_SHEETS_URL, {
            action: 'single',
            customer: customer
        });
     ...
     */
    static async exportCustomersToExcel(searchTerm, statusFilter, dateFilter) {
        const filters = {
            searchTerm,
            status: statusFilter,
        ...
        const result = await customer_1.CustomerService.list(filters, 1000, 0); // Get up to 1000 customers
     ...
     */
    static async exportCustomersToPDF(searchTerm, statusFilter, dateFilter) {
        const filters = {
            searchTerm,
            status: statusFilter,
            startDate: dateFilter?.start ? new Date(dateFilter.start) : undefined,
        ...
        const result = await customer_1.CustomerService.list(filters, 1000, 0);
        const customers = result.customers;
        const doc = new jspdf_1.default();
        // Title
        doc.setFontSize(20);
     ...
     */
    static async exportCustomersToGoogleSheets(searchTerm, statusFilter, dateFilter) {
        const filters = {
            searchTerm,
            status: statusFilter,
            startDate: dateFilter?.start ? new Date(dateFilter.start) : undefined,
     ...
     */
    static formatPriorityFlags(flags) {
        const priorities = [];
        if (flags.senior_citizen)
            priorities.push('Senior Citizen');
        if (flags.pregnant)
            priorities.push('Pregnant');
        if (flags.pwd)
            priorities.push('PWD');
...
ExportService.GOOGLE_SHEETS_URL = config_1.config.GOOGLE_SHEETS_URL;
