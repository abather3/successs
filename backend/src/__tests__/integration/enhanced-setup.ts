...
import { UserRole, UserStatus, QueueStatus, PaymentMode, PaymentStatus } from '../../types';
...
export class EnhancedTestSetup {
  ...
  async createTestSchema(schemaName: string) {
  ...
  private async createTables(client: any, schemaName: string) {
    // Users table
    await client.query(`
  ...
  async createTestUsers(schemaName: string) {
    const client = await pool.connect();
    try {
      // Create super admin
      const superAdminResult = await client.query(`
        ...
        VALUES ($1, $2, $3, $4, $5)
        RETURNING id
      `, ['superadmin@test.com', '$2b$10$hashedpassword', 'Super Admin Test', UserRole.SUPER_ADMIN, UserStatus.ACTIVE]);
      ...
      const adminResult = await client.query(`
        ...
        VALUES ($1, $2, $3, $4, $5)
        RETURNING id
      `, ['admin@test.com', '$2b$10$hashedpassword', 'Admin Test', UserRole.ADMIN, UserStatus.ACTIVE]);
      ...
      const salesAgent1Result = await client.query(`
        ...
        VALUES ($1, $2, $3, $4, $5)
        RETURNING id
      `, ['sales1@test.com', '$2b$10$hashedpassword', 'Sales Agent 1', UserRole.SALES, UserStatus.ACTIVE]);

      const salesAgent2Result = await client.query(`
        INSERT INTO ${schemaName}.users (email, password, full_name, role, status)
        VALUES ($1, $2, $3, $4, $5)
        RETURNING id
      `, ['sales2@test.com', '$2b$10$hashedpassword', 'Sales Agent 2', UserRole.SALES, UserStatus.ACTIVE]);
      ...
      const cashier1Result = await client.query(`
        ...
        VALUES ($1, $2, $3, $4, $5)
        RETURNING id
      `, ['cashier1@test.com', '$2b$10$hashedpassword', 'Cashier 1', UserRole.CASHIER, UserStatus.ACTIVE]);

      const cashier2Result = await client.query(`
        INSERT INTO ${schemaName}.users (email, password, full_name, role, status)
        VALUES ($1, $2, $3, $4, $5)
        RETURNING id
      `, ['cashier2@test.com', '$2b$10$hashedpassword', 'Cashier 2', UserRole.CASHIER, UserStatus.ACTIVE]);
  ...
  async createTestCustomers(schemaName: string, salesAgentId: number, count: number = 5) {
    const client = await pool.connect();
    ...
    try {
      for (let i = 1; i <= count; i++) {
        ...
        `, [
          ...
          JSON.stringify({ mode: PaymentMode.CASH, amount: 1000 + (i * 100) }),
  ...
  async createTestTransactions(schemaName: string, customerIds: number[], salesAgentId: number, cashierId: number) {
    const client = await pool.connect();
  ...
  async cleanupTestSchema(schemaName: string) {
    const client = await pool.connect();
    try {
  ...
  async simulateNetworkDelay(min: number = 100, max: number = 500): Promise<void> {
    const delay = Math.floor(Math.random() * (max - min + 1)) + min;
    return new Promise(resolve => setTimeout(resolve, delay));
  }

  async simulateNetworkFailure(failureRate: number = 0.1): Promise<boolean> {
