...
describe('Chaos Testing Suite', () => {
  ...
  describe('Network Failure Simulations', () => {
    it('should handle intermittent network failures during payment processing', async () => {
      const client = ioClient('http://localhost:3004');
    ...
    it('should maintain data consistency during network partitions', async () => {
      ...
      try {
        ...
        await client.query(`
          ...
          WHERE id = $2
        `, [PaymentStatus.PARTIAL, transactionId]);
        ...
        expect([PaymentStatus.UNPAID, PaymentStatus.PARTIAL]).toContain(transaction.rows[0].payment_status);
      ...
      try {
        ...
        while (retryCount < maxRetries && !paymentSuccess) {
          ...
          if (!timeoutOccurred) {
            ...
            `, [transactionId, 500, PaymentMode.GCASH, userIds.cashier1Id, `Retry ${retryCount} successful`]);
    ...
    it('should handle transaction rollback on database failures', async () => {
      ...
      try {
        ...
        await client.query(`
          ...
          SET payment_status = $1
          WHERE id = $2
        `, [PaymentStatus.PROCESSING, transactionId]);

        await client.query(`
          INSERT INTO ${testSchema}.payment_settlements (
            transaction_id, amount, payment_mode, cashier_id
          ) VALUES ($1, $2, $3, $4)
        `, [transactionId, 250, PaymentMode.CASH, userIds.cashier1Id]);
        ...
        if (dbFailed) {
          ...
          expect(finalTransaction.rows[0].payment_status).not.toBe(PaymentStatus.PROCESSING);
        }
