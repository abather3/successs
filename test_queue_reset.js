...
async function testQueueReset() {
  ...
  try {
    ...
    currentCustomers.rows.forEach(customer => {
      console.log(`   - ID: ${customer.id}, Queue: ${customer.queue_number}, Status: ${customer.status}, Created: ${customer.created_at}, Served: ${customer.served_at || 'Not served'}`);
    });
    console.log();
    ...
    archivedToday.rows.forEach(customer => {
      console.log(`   - Customer ID: ${customer.customer_id}, Queue: ${customer.queue_number}, Status: ${customer.status}, Archived: ${customer.archived_at}`);
    });
    console.log();
    ...
    const affectedCustomers = await client.query(`
      SELECT c.id, c.queue_number, c.status, c.created_at, c.served_at
      FROM customers c
    ...
    analyticsToday.rows.forEach(record => {
      console.log(`   - Hour: ${record.date_hour}, Customers: ${record.customers_served}, Avg Wait: ${record.avg_wait_time || 'N/A'}min`);
    });
    console.log();
